...
export function renderDebitNoteHtml(sale, company) {
  ...
  const dnDate = sale.debitNoteDate || new Date();
  const dnEnDate = new Date(dnDate).toLocaleDateString('en-GB', { year: 'numeric', month: '2-digit', day: '2-digit' });
  const dnMiti = getMiti(dnDate);

  const sellerName = sale.customer?.name || 'Cash Sale / Walk-in';
  const sellerAddress = sale.customer?.address || '-';
  const sellerPan = sale.customer?.pan || '';
  const panDigits = (sellerPan || '').replace(/\D/g, '').padEnd(9, ' ').slice(0, 9).split('');

  const buyerPan = company?.pan || '';
  const buyerPanDigits = (buyerPan || '').replace(/\D/g, '').padEnd(9, ' ').slice(0, 9).split('');
  const buyerPanBoxes = buyerPanDigits.map(d => `<div class="pan-box">${escapeHtml(d.trim())}</div>`).join('');
  ...
  <div class="meta-section">
    <div class="meta-section">
      <div class="row-flex">
        <div><strong>Debit Note No:</strong> ${escapeHtml(sale.debitNoteNumber || '-')}</div>
        <div><strong>Date:</strong> ${escapeHtml(dnMiti || dnEnDate)} (${escapeHtml(dnEnDate)})</div>
      </div>
      ...
      <div style="margin-top: 6px;"><strong>Buyer's PAN No:</strong> <div class="pan-boxes">${buyerPanBoxes}</div></div>
      <div><strong>Seller's Name:</strong> ${escapeHtml(sellerName)}</div>
      <div><strong>Seller's Address:</strong> ${escapeHtml(sellerAddress)}</div>
      <div style="margin-top: 4px;">
        <strong>Seller's PAN No:</strong>
        <div class="pan-boxes">${panBoxes}</div>
      </div>
      ${sale.refundRemark ? `<div style="margin-top: 6px;"><strong>Reason for Return:</strong> ${escapeHtml(sale.refundRemark)}</div>` : ''}
    </div>
        ...
        <tr>
          ...
          <td colspan="3" style="padding: 0;">
            <table class="summary-table">
              ...
              <tr>
                <td class="summary-label">Total Debit Amount</td>
    ...
    <div class="signatures-section">
      <div>
        ...
        <div style="margin-top: 15px;">
          <strong>Seller's signature:</strong> <span class="sig-line-dotted" style="display:inline-block; width: 120px;"></span>
        </div>
      </div>
      <div class="signature-box">
        <div style="margin-bottom: 25px;">
          <u><strong>Buyer's Signature (or For ${escapeHtml(company?.name || '')})</strong></u>
        </div>
...
export function printDebitNote(sale, company) {
  if (!sale) return;
  if (!company) { company = JSON.parse(localStorage.getItem('user') || '{}').company || {}; }
  const bodyHtml = renderDebitNoteHtml(sale, company);
...
<head>
<meta charset="UTF-8">
<title>Debit Note ${sale.debitNoteNumber || ''}</title>
<style>
...
.doc-title { font-size: 18px; font-weight: bold; text-transform: uppercase; text-decoration: underline; margin-top: 6px; color: #1d4ed8; }
...
table.invoice-table th { font-weight: bold; text-align: center; font-size: 12px; background-color: #eff6ff; }
...
<body>
  <div class="invoice-container">
    <div class="header">
      ...
      <div class="doc-title">Debit Note (डेबिट नोट)</div>
    </div>
    ${bodyHtml}
