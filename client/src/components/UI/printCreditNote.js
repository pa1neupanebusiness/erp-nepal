...
export function renderCreditNoteHtml(sale, company) {
  ...
  const cnDate = sale.creditNoteDate || new Date();
  ...
  <div class="meta-section">
    <div class="meta-section">
      <div class="row-flex">
        <div><strong>Credit Note No:</strong> ${escapeHtml(sale.creditNoteNumber || '-')}</div>
        ...
        <tr>
          ...
          <td colspan="3" style="padding: 0;">
            <table class="summary-table">
              ...
              <tr>
                <td class="summary-label">Total Credit Amount</td>
...
export async function printCreditNote(sale, company) {
  if (!sale) return;
  if (!company) { company = JSON.parse(localStorage.getItem('user') || '{}').company || {}; }
  const bodyHtml = renderCreditNoteHtml(sale, company);
...
<head>
<meta charset="UTF-8">
<title>Credit Note ${sale.creditNoteNumber || ''}</title>
<style>
...
.doc-title { font-size: 18px; font-weight: bold; text-transform: uppercase; text-decoration: underline; margin-top: 6px; color: #b91c1c; }
...
table.invoice-table th { font-weight: bold; text-align: center; font-size: 12px; background-color: #fef2f2; }
...
<body>
  <div class="invoice-container">
    <div class="header">
      ...
      <div class="doc-title">Credit Note (क्रेडिट नोट)</div>
    </div>
    ${bodyHtml}
