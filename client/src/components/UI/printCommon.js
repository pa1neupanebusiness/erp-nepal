...
.meta-grid { display: flex; justify-content: space-between; gap: 12px; margin-bottom: 15px; line-height: 1.5; }
 ...
 */
export async function openPrintWindow({ title, company, subtitle, docTitle, bodyHtml, landscape = false, showTimestamp = isTimestampEnabled() } = {}) {
  const c = company || {};
  ...
  if (showTimestamp) {
    ...
    timestampHtml = `<div class="footer-text">${esc(formatTimestamp(ts, { nepali: true }))}</div>`;
  }
...
export async function printHtmlDocument(html, title = 'Report', { showTimestamp = isTimestampEnabled() } = {}) {
  const iframe = document.createElement('iframe');
  iframe.setAttribute('aria-hidden', 'true');
  iframe.style.position = 'fixed';
  iframe.style.top = '0';
  ...
  if (showTimestamp) {
    const ts = await getSystemTime();
    timestampHtml = `<div class="footer-text" style="margin-top:12px;">${esc(formatTimestamp(ts, { nepali: true }))}</div>`;
  }

  const idoc = iframe.contentWindow.document;
  idoc.open();
  idoc.write(`<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><title>${esc(title)}</title><style>${INVOICE_CSS}</style></head><body><div class="invoice-card">${html}${timestampHtml}</div></body></html>`);
  idoc.close();
